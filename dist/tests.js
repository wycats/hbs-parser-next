...
qunit_1.test("match: one character tag", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, fragment] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.tag("h"), input));
    helpers_1.eqSnippet(next, input.slice(1).rest);
    helpers_1.eqSnippet(fragment, input.slice(1));
});
qunit_1.test("match: multi-character tag", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, fragment] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.tag("hello"), input));
    helpers_1.eqSnippet(next, input.slice(5).rest);
    helpers_1.eqSnippet(fragment, input.slice(5));
});
qunit_1.test("mismatch: multi-character tag", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let next = input.invoke(hbs_parser_next_1.combinators.tag("holla"), input);
    helpers_1.eqError(next, hbs_parser_next_1.err(input, "tag"));
});
qunit_1.test("mismatch: not at 0 offset", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.tag("hello "), input));
...
qunit_1.test("match: skipping a chunk of characters", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeUntil("world"), input));
    helpers_1.eqSnippet(next, input.slice(6).rest);
    helpers_1.eqSnippet(match, input.slice(6));
});
qunit_1.test("match: skipping zero characters", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeUntil("hello"), input));
    helpers_1.eqSnippet(next, input.rest);
    helpers_1.eqSnippet(match, input.rest);
});
qunit_1.test("match: skipping until the last character", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeUntil("d"), input));
    helpers_1.eqSnippet(next, input.slice(10).rest);
    helpers_1.eqSnippet(match, input.slice(10));
});
qunit_1.test("match: skipping until the last characters", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeUntil("world"), input));
    helpers_1.eqSnippet(next, input.slice(6).rest);
    helpers_1.eqSnippet(match, input.slice(6));
});
qunit_1.test("mismatch: no match before the end", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeUntil("cruel"), input));
    helpers_1.eqSnippet(next, input.slice(11).rest);
    helpers_1.eqSnippet(match, input.slice(11));
});
qunit_1.module("[combinators] takeWhile");
qunit_1.test("match: at non-zero offset", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello!!!!", LOGGER);
    let [next1] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.tag("hello"), input));
...
qunit_1.test("match: skipping zero characters", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeWhile("hello"), input));
    helpers_1.eqSnippet(next, input.slice(5).rest);
    helpers_1.eqSnippet(match, input.slice(5));
});
qunit_1.test("match: skipping until the last characters", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeWhile("hello world"), input));
    helpers_1.eqSnippet(next, input.slice(11).rest);
    helpers_1.eqSnippet(match, input.slice(11));
});
qunit_1.test("mismatch: no match before the end", assert => {
    let input = hbs_parser_next_1.Snippet.input("hellohello", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.combinators.takeWhile("hellohello"), input));
    helpers_1.eqSnippet(next, input.slice(10).rest);
    helpers_1.eqSnippet(match, input.slice(10));
});
...
qunit_1.test("zero times", assert => {
    let input = hbs_parser_next_1.Snippet.input("abcabcabc", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("def")), input));
    helpers_1.eqSnippet(next, input);
    helpers_1.eqSnippets(match, []);
});
qunit_1.test("one time", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("hello")), input));
    helpers_1.eqSnippet(next, input.slice(5).rest);
    helpers_1.eqSnippets(match, [input.slice(5)]);
});
qunit_1.test("several times", assert => {
    let input = hbs_parser_next_1.Snippet.input("abcabcabc", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("abc")), input));
    helpers_1.eqSnippet(next, input.slice(9).rest);
    helpers_1.eqSnippets(match, [
        input.slice(3),
...
qunit_1.test("zero times", assert => {
    let input = hbs_parser_next_1.Snippet.input("abcabcabc", LOGGER);
    let mismatch = input.invoke(hbs_parser_next_1.utils.present(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("def"))), input);
    helpers_1.eqError(mismatch, hbs_parser_next_1.err(input, "empty"));
});
qunit_1.test("one time", assert => {
    let input = hbs_parser_next_1.Snippet.input("hello world", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.utils.present(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("hello"))), input));
    helpers_1.eqSnippet(next, input.slice(5).rest);
    helpers_1.eqSnippets(match, [input.slice(5)]);
});
qunit_1.test("several times", assert => {
    let input = hbs_parser_next_1.Snippet.input("abcabcabc", LOGGER);
    let [next, match] = helpers_1.unwrap(input.invoke(hbs_parser_next_1.utils.present(hbs_parser_next_1.multi.many(hbs_parser_next_1.combinators.tag("abc"))), input));
    helpers_1.eqSnippet(next, input.slice(9).rest);
    helpers_1.eqSnippets(match, [
        input.slice(3),
        input.slice(3).slice(3),
